...
from utilsfixedUG import (compute_inv_K_theta0,
...
class MAGMA:
    ...
    def set_TY(self, T: Union[np.ndarray, list], Y: Union[np.ndarray, list]) -> None:
        ...
        if self.common_grid_flag:
            ...
            n_individuals = len(Y)
            T_masks = np.zeros((n_individuals, self.n_T))
            Y_normalized = np.zeros((n_individuals, self.n_T))

            for i in range(n_individuals):
                Yi_sorted = Y[i][np.argsort(T[i])]
                Ti_sorted = np.sort(T[i])

                mask = np.isin(self.T, Ti_sorted)
                Yi_normalized = np.zeros(self.n_T)
    ...
    def set_m0(self, m0: Union[int, float, list, np.ndarray], m0_function: Callable) -> None:
        ...
        if m0 is None:
            m0 = m0_function(self.n_T)
        elif isinstance(m0, (int, float)):
            m0 = m0 * np.ones(self.n_T)
        elif isinstance(m0, (list, tuple)):
            m0 = np.array(m0)
        assert isinstance(m0, np.ndarray) and len(m0) == self.n_T
        self.m0 = m0
    ...
    def E_step(self):
        ...
        if self.common_hp_flag and self.common_grid_flag:
            ...
            inv_Psi_Theta_Sigma_dot_Y = np.zeros(len(self.T))
            inv_Psi_Theta_Sigma = np.zeros(inv_K_theta0.shape)
            ...
            for i in range(self.n_individuals):
                ...
                if self.common_grid_flag: 
                    mask, tilde_Yi = None, self.Y[i]
                else: 
                    _, _, mask, tilde_Yi = self.get_individual(i)
    ...
    def M_step(self):
        ...
        theta0 = scipy.optimize.minimize(
            fun=lambda x: log_likelihood_theta0(x, self.kernel_k, self.T, 
